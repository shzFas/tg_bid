...
export async function notifySpecialist(tgId: number, requestId: number) {
    const botToken = process.env.SPEC_BOT_TOKEN;

    const text = `
<b>🔔 Заявка обновлена</b>

Заявка ID <b>${requestId}</b> была изменена администратором.
Пожалуйста, проверьте новые данные.
  `.trim();
